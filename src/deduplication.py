...
def deduplicate(df):
    ...
    rejected_id = df[df.duplicated(subset=['Transaction ID'], keep='first')] # get actual duplicates
    ...
    rejected_unique = deduped_id[deduped_id.duplicated(subset=unique_identifiers, keep='first')] # get rows that were removed again
